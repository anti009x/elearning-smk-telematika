<div class="bg-blue-700 px-4 sm:px-6 py-3 flex items-center justify-between shadow h-16 fixed top-0 left-0 right-0 z-50">
    <div class="flex items-center text-white">
        <div class="h-6 w-8 mr-2 fill-current">
            <img src="{{ asset('images/logo.png') }}" class="img-circle elevation-2" alt="User Image" width="100"
                height="120" margin="absolut">
        </div>
        <span class="font-semibold text-sm sm:text-xl tracking-tight" style="margin-top: 7px">SMK TELEMATIKA</span>
    </div>
    <div class="relative">
        @auth
            <div class="flex items-center cursor-pointer" id="opennavdropdown">
                <img class="w-8 h-8 rounded-full mr-2"
                    src="{{ asset('images/profile/' . auth()->user()->profile_picture) }}" alt="Avatar">
                ...
                <div class="hidden h-24 w-48" id="navdropdown">
                    <div class="px-8 py-4 border-t border-blue-800">
                        <a href="{{ route('profile') }}" class="flex items-center pb-3 text-sm text-gray-200 font-semibold">
                            <svg class="h-4 w-4 mr-2 fill-current text-gray-200" xmlns="http://www.w3.org/2000/svg"
                                viewBox="0 0 576 512">
                                <path
                        ...
                        <form action="{{ route('logout') }}" method="POST" class="pb-2">
                            @csrf
                            <button class="flex items-center text-sm text-gray-200 font-semibold focus:outline-none"
                                type="submit">
                                <svg class="h-4 w-4 mr-2 fill-current text-gray-200" xmlns="http://www.w3.org/2000/svg"
                                    viewBox="0 0 512 512">
                                    <path
                ...
                @if (Route::has('login'))
                    <div>
                        <a class="flex items-center mr-4 text-sm text-gray-200 font-semibold" href="{{ route('login') }}">
                            <svg class="h-3 w-3 mr-1 fill-current text-gray-200" aria-hidden="true" focusable="false"
                                data-prefix="fas" data-icon="sign-in-alt" class="svg-inline--fa fa-sign-in-alt fa-w-16"
