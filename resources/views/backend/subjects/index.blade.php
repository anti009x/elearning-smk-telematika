    ...
    <div class="roles-permissions">
        <div class="flex items-center justify-between mb-6">
            ...
            <div class="flex flex-wrap items-center">
                <a href="{{ route('subject.create') }}"
                    ...
                    <span class="ml-2 text-xs font-semibold">Tambah Subjek Baru</span>
                </a>
            </div>
        </div>
        ...
        <div class="mt-8 bg-white rounded border-b-4 border-gray-300">
            <div
                ...
                <div class="w-2/12 px-4 py-3">Kode</div>
                <div class="w-2/12 px-4 py-3">Guru</div>
                <div class="w-3/12 px-4 py-3">Keterangan</div>
                <div class="w-2/12 px-4 py-3 text-right">Aksi</div>
            </div>
            @foreach ($subjects as $subject)
                <div class="flex flex-wrap items-center text-gray-700 border-t-2 border-l-4 border-r-4 border-gray-300">
                    <div class="w-3/12 px-4 py-3 text-sm font-semibold text-gray-600 tracking-tight">{{ $subject->name }}
                    </div>
                    <div class="w-2/12 px-4 py-3 text-sm font-semibold text-gray-600 tracking-tight">
                        {{ $subject->subject_code }}</div>
                    <div class="w-2/12 px-4 py-3 text-sm font-semibold text-gray-600 tracking-tight">
                        {{ $subject->teacher ? ($subject->teacher->user ? $subject->teacher->user->name : 'No User') : 'No Teacher' }}
                    </div>
