...
@section('content')
    <div class="roles-permissions">
        <div class="flex items-center justify-between mb-6">
            <div>
                <h2 class="text-gray-700 uppercase font-bold bg-blue-600 text-white px-4 py-2 rounded " style="color: white">
                    SISWA</h2>
            </div>
            <div class="flex flex-wrap items-center">
                <a href="{{ route('student.create') }}"
                    class="bg-green-500 text-white text-sm uppercase py-2 px-4 flex items-center rounded">
                    <svg class="w-3 h-3 fill-current" aria-hidden="true" focusable="false" data-prefix="fas" data-icon="plus"
                    ...
                    <span class="ml-2 text-xs font-semibold">TAMBAH SISWA</span>
                </a>
            </div>
        </div>
        ...
        <div class="mt-8 bg-white rounded border-b-4 border-gray-300">
            <div
                ...
                <div class="w-2/12 px-4 py-3">Kelas</div>
                <div class="w-2/12 px-4 py-3">Nomor Telepon</div>
                <div class="w-2/12 px-4 py-3 text-right">Aksi</div>
            </div>
            @foreach ($students as $student)
                <div class="flex flex-wrap items-center text-gray-700 border-t-2 border-l-4 border-r-4 border-gray-300">
                    ...
                    <div class="w-2/12 px-4 py-3 text-sm font-semibold text-gray-600 tracking-tight">{{ $student->phone }}
                    </div>
                    <div class="w-2/12 flex items-center justify-end px-3">
                        <a href="{{ route('student.show', $student->id) }}"
                            class="ml-1 bg-blue-600 block p-1 border border-blue-600 rounded-sm" title="View Profile">
                        ...
                        <a href="{{ route('student.destroy', $student->id) }}" data-url="{!! URL::route('student.destroy', $student->id) !!}"
                            class="deletestudent ml-1 bg-red-600 block p-1 border border-red-600 rounded-sm" title="Delete">
                            <svg class="h-3 w-3 fill-current text-gray-100" aria-hidden="true" focusable="false"
