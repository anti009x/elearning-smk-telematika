...
@section('content')
    <div class="create">
        ...
        <div class="flex items-center justify-between mb-6">
            <div>
                <h2 class="text-gray-700 uppercase font-bold"> Kehadiran Kelas :- {{ $class->class_name }}</h2>
            </div>
        ...
        <div class="w-full mt-8 bg-white rounded">
            ...
            <div class="w-full px-6 py-6">
                <div class="flex items-center bg-gray-600 rounded-tl rounded-tr">
                    <div class="w-4/12 text-left text-white py-2 px-4 font-semibold">Siswa</div>
                    <div class="w-3/12 text-left text-white py-2 px-4 font-semibold">Nomor</div>
                    <div class="w-5/12 text-right text-white py-2 px-4 font-semibold">Aksi</div>
                </div>
                <form action="{{ route('teacher.attendance.store') }}" method="POST">
                    @foreach ($class->students as $student)
                        <div class="flex items-center justify-between border border-gray-200">
                            @csrf
                            <div class="w-4/12 text-sm text-left text-gray-600 py-2 px-4 font-semibold">
                                {{ $student->user->name }}</div>
                            <div class="w-3/12 text-sm text-left text-gray-600 py-2 px-4 font-semibold">
