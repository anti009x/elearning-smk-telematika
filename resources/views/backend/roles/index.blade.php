        ...
        <div class="flex items-center justify-between mb-6">
            ...
            <div class="flex flex-wrap items-center">
                <a href="{{ route('role.create') }}"
                    class="bg-green-500 text-white text-sm uppercase py-2 px-4 mr-2 flex items-center rounded">
        ...
        <div class="mt-8 bg-white rounded border-b-4 border-gray-300">
            ...
            @foreach ($roles as $role)
                <div class="flex flex-wrap items-center text-gray-700 border-t-2 border-l-4 border-r-4 border-gray-300">
                    <div class="w-3/12 px-4 py-3 text-sm font-semibold text-gray-600 tracking-tight">{{ $role->name }}
                    </div>
                    <div class="w-7/12 px-4 py-3 flex flex-wrap">
                        @foreach ($role->permissions as $permission)
                            <span
                                class="bg-gray-200 text-sm mr-1 mb-1 px-2 border rounded-full">{{ $permission->name }}</span>
                        @endforeach
                    </div>
