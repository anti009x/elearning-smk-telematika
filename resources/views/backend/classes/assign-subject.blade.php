                    ...
                    <div class="md:w-2/3 block text-gray-600 font-bold">
                        @foreach ($subjects as $subject)
                            <div class="flex items-center">
                                <label>
                                    <input name="selectedsubjects[]" class="mr-2 leading-tight" type="checkbox"
                                        value="{{ $subject->id }}"
                                        @foreach ($assigned->subjects as $item)
                                            {{ $item->id === $subject->id ? 'checked' : '' }} @endforeach>
                                    <span class="text-sm">
                                        {{ $subject->name }}
                                    </span>
                                </label>
        ...
        <div class="w-full py-12">
            ...
            @foreach ($assigned->students as $student)
                <div class="flex items-center justify-between border border-gray-200 mb-px">
                    ...
                    <div class="w-1/4 text-right text-gray-600 py-2 px-4 font-medium">{{ $student->parent->user->name }}
                    </div>
                </div>
            @endforeach
        </div>
