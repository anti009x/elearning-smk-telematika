...
<div class="w-full block mt-4 sm:mt-8">
    <div class="flex flex-wrap sm:flex-no-wrap justify-between">
        @foreach ($parents->children as $key => $children)
            <div
                class="w-full bg-gray-200 text-center border border-gray-300 rounded px-8 py-6 mb-4 {{ $key >= 1 ? 'ml-0 sm:ml-2' : '' }} {{ $parents->children_count === 1 ? 'sm:max-w-sm' : '' }}">
                <div class="text-gray-700 font-bold">
                    ...
                    <div class="mt-6">
                        <a href="{{ route('attendance.show', $children->id) }}"
                            class="bg-blue-600 inline-block mb-4 text-sm text-white uppercase font-semibold px-4 py-2 border border-gray-400 rounded">Attendence
                            Report</a>
                    </div>
                </div>
            </div>
